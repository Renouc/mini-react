...
describe("workLoop测试", () => {
  test("测试Fiber构建", () => {
    ...
    expect(container_fiber.child?.stateNode?.textContent).toBe(
      "Hello,my react!!!"
    );
    // 测试p节点
    ...
    expect(container_fiber.child?.sibling?.stateNode?.childNodes.length).toBe(
      2
    );
    // 测试p节点中的第一个子节点
    ...
    expect(
      container_fiber.child?.sibling?.child?.sibling?.stateNode?.tagName
    ).toBe("SPAN");
    expect(
      container_fiber.child?.sibling?.child?.sibling?.stateNode?.textContent
    ).toBe("span text");
  });
});
